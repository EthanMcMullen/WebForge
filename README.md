...
4. Firecrawl Search discovers public candidate pages for automatic jobs and excludes known unsupported domains. Price jobs skip known search/category pages; when an Amazon search result already contains a product ASIN in a `/clp/` URL, discovery turns it into the corresponding `/dp/` detail candidate for source review rather than guessing a new product. Provided URL jobs skip search and never switch to other sources.
5. A separate worker claims the queued run. Firecrawl Scrape extracts the confirmed fields plus an internal identity field that is not served through the public API. When text scraping returns nothing usable (empty render, no structured JSON, malformed values), a requested price is missing or unsupported by text, or the page blocks text extraction (retailer bot walls such as Amazon), the worker takes up to two vision fallbacks per run: it screenshots the page with Firecrawl over a stealth proxy and reads the same fields off the screenshot with a vision-capable OpenAI model. Vision prices require the exact item's name and current amount together in page text or a short visual transcription; all results still pass relevance and quality checks. Discovery skips known search and category pages for price requests, and Amazon prices require a product detail URL even for vision. Every screenshot attempt counts against the run's scrape ceiling. Set `WEBFORGE_VISION_FALLBACK=0` to disable it, or `OPENAI_VISION_MODEL` to override the model.
...
Manual and scheduled refreshes bypass Firecrawl's indexed scrape cache for source pages and vision screenshots. A refresh may still return the same price if the live page has not changed; compare the record's extraction timestamp or the latest run to confirm it completed.
