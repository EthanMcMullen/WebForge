...
import type { ApiJob, ApiJobStatus, ApiRecord, RunOutcome, RunSummary } from "./types";
...
async function enrichedJob(job: JobDocument | null): Promise<ApiJob | null> {
  ...
  return { ...job, runSummary, recordCount };
}
...
export async function updateApiJobSettings(jobId: string, input: { name?: string; refreshInterval?: number | null }): Promise<ApiJob> {
  const { jobs } = await collections();
  const job = await jobs.findOne({ id: jobId });
  if (!job) throw new Error("API job not found.");
  const now = new Date().toISOString();
  ...
  const updated = await jobs.updateOne({ id: jobId, status: { $nin: activeStatuses } }, { $set: {
    name: input.name ?? job.name, refreshInterval: interval, nextRefreshAt: next,
