...
export async function planApiJob(userRequest: string, combineRequested = false): Promise<ApiPlan> {
  ...
  const makePlan = async (feedback?: string) => client.responses.create({
    ...
    instructions: [
      ...
      "For a collection such as all courses or different espresso machines, set recordScope collection and combineSources false. One record represents one member. Search broadly for real individual item pages; do not choose a specific course or model as a stand-in for the whole collection. For explicitly named individual items, use a query per item. When combining sources, queries target complementary field groups for one exact entity.",
